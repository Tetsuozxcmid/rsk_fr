...
export default function LoginStage1({ onBack, pageVariants, custom = 1 }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const emailOrLogin = email.trim();
        if (!emailOrLogin) return;
        ...
        try {
            ...
            alert("Новый пароль отправлен на почту. Проверьте входящие и спам.");
            setTimer(300);
        } catch (err) {
    ...
    return (
        <motion.div key="login-stage1" custom={custom} initial="initial" animate="in" exit="out" variants={pageVariants} className="auth_cntr col-span-4 absolute w-full h-full justify-center">
            <div className="flex flex-col items-center gap-[0.5rem]">
                ...
                <p className="text-(--color-gray-black) text-center">Мы отправим на почту новый пароль. Укажите email или логин, привязанный к аккаунту. Повторно отправить можно через 5 минут.</p>
            </div>
            <form id="recovery" onSubmit={handleSubmit} className="w-full grid grid-cols-1 gap-[0.75rem]">
                <Input type="text" autoComplete="username" placeholder="Почта или логин" value={email} onChange={(e) => setEmail(e.target.value)} required />
